...
s3_model_path = 's3://warehouse-algo/sequence_model_predict_best_model_iq_no_cancel/ds=%s/'
s3_pred_path = 's3://warehouse-algo/sequence_model_predict_result_iq_no_cancel/ds=%s/'
s3_eval_path = 's3://warehouse-algo/sequence_model_evaluated_result_iq_no_cancel/ds=%s/evaluated_result.parquet'
...
if __name__ == '__main__':
    ...
    if args.range != '':
        for i in args.range.split(','):
            ...
            main(args)
            alert_feishu(f"iq_no_cancel_pipeline process :{args.time_delta}")
    else:
        main(args)
        alert_feishu(f"iq_no_cancel_pipeline process :{args.time_delta}")
