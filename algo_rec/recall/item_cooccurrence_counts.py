...
def main(args):
    ...
    m = {}
    for uuid, goods_id, clk_num, country_code in zip(pt['uuid'], pt['goods_id'], pt['clk_num'],
                                                     pt['country_code'][0:100000]):
        item_id = int(goods_id.as_py())
        u = uuid.as_py()
        if item_id in m:
            m[item_id].add(u)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--in_file', type=str,
                        default='s3://warehouse-algo/rec/cn_rec_detail_recall_ui_relation/ds=20250421')
