...
def build_tfrecord(path_pt_list, path_tfr_local_list, path_tfr_s3_list,proc_id,stat_file,stat_flag,
    ...
    def build_mt(tt, feature, stat_d, stat_flag, itm_d):
        mt = []
        mt_w = []
        main_goods = int(tt['main_goods_id'])
        tgt_id = int(tt['goods_id'])
        if main_goods in itm_d:
            main_cate_id = str(itm_d[main_goods]['cate_id'])
        else:
            main_cate_id = "0"
    ...
    def build_feature(t, feature):
        ...
        item_features_double ={ "gmv_price": 0.0}
        ...
        for name in item_features_int.keys():
            if itm_id_int in itm_d:
                ...
                if raw_v is not None:
                    ...
                    feature.update({name: ints_fea(item_features_int[name])})
            else:
                feature.update({name: ints_fea(item_features_int[name])})
        ...
        for name in item_features_double.keys():
            if itm_id_int in itm_d:
                ...
                if raw_v is not None:
                    ...
                    feature.update({name: floats_fea(item_features_double[name])})
            else:
                feature.update({name: floats_fea(item_features_double[name])})

        main_itm_id_int = int(t['main_goods_id'])
        ...
        for name in main_item_features_int.keys():
            ...
            if main_itm_id_int in itm_d:
                ...
                if raw_v is not None:
                    ...
                    feature.update({name: ints_fea(main_item_features_int[name])})
            else:
                feature.update({name: ints_fea(main_item_features_int[name])})
    ...
    stat_d = {"sample_id":[], "s":[], "is_clk":[], "is_cart":[], "is_wish":[], "is_pay":[], "pos_idx":[]}
    for pt_file, tfr_local_file, tfr_s3_file in zip(path_pt_list, path_tfr_local_list, path_tfr_s3_list):
        ...
        for t in pt:
            ...
            if stat_flag:
                ...
                stat_d['pos_idx'].append(t['pos_idx'])
            build_mt(t, feature, stat_d, stat_flag, itm_d)
