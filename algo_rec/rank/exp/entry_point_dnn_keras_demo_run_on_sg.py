...
# To get numpy output in eval. # TODO
...
site = 'in'
model = 'in_ctr_dnn_keras_debug_v0'
tfr_sample_dir = 'rec/cn_rec_detail_sample_v0_tfr-all/'
...
# train_days = 40
train_range = ['20241101', '20241102']
target_date = train_range[-1]
from sagemaker.inputs import TrainingInput

sg_estimator = TensorFlow(
        entry_point='entry_point_dnn_keras_demo.py',
        dependencies=[],
        role=role,
        input_mode="Pipe",
        instance_count=1,
        ...
        hyperparameters={
            "mode": "train",
            "epochs":1,
            # "checkpoint": make_sure_bucket_prefix(job_dir + "checkpoint"), # re-use model
            "checkpoint": job_dir_s3 + 'ds=' + target_date + '/checkpoint',
            "linear_lr": 0.005 * 9,
            "dnn_lr": 0.01 * 9,
            "batch_size": 1024,
            # "batch_norm": False,
...
train_params = {'inputs': {'train': 's3://warehouse-algo/rec/cn_rec_detail_sample_v0_tfr-all/ds=20241101',
                               'eval': 's3://warehouse-algo/rec/cn_rec_detail_sample_v0_tfr-all/ds=20241102',
                              },
                    'job_name': 'Job-laidehe-test-%s-%s' % (model.replace('_', '-'),ts2date(time.time(), '%m-%d-%H-%M-%S'))}
print('Train params: ', train_params)
# train_s3_input = TrainingInput('s3://warehouse-algo/rec/cn_rec_detail_sample_v0_tfr-all/ds=20241101')
